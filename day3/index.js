...
// const firstWireRoute = [[]];
// const secondWireRoute = [[]];
...
const firstWireRoute = new Map();
const secondWireRoute = new Map();

drawRoute(firstWire, firstWireRoute);
drawRoute(secondWire, secondWireRoute);
const intersection = getIntersection(firstWireRoute, secondWireRoute);
console.log(intersection);
console.log(
  getLesserStepsIntersection(intersection, firstWireRoute, secondWireRoute)
);

function drawRoute(wire, map) {
  let directions = wire.reduce((acc, current) => {
    let [direction, moves] = current.match(/[a-z]+|[^a-z]+/gi);
...
function getIntersection(map1, map2) {
  return [...map1.keys()].filter((item) => map2.has(item));
}

function getClosestToCenter(coordinates) {
  return coordinates
    .map((item) => {
      let [x, y] = item.split('#').map((item) => parseInt(item));
...
function getLesserStepsIntersection(coordinates, map1, map2) {
  return coordinates
    .map((item) => {
      return map1.get(item) + map2.get(item);
    })
    .sort((a, b) => {
      return a - b;
    })[0];
...
// function insertCoordinateInSet(instruction, currentPosition, array) {
//   let [direction, moves] = instruction.match(/[a-z]+|[^a-z]+/gi);
...
// function moveUp(x, y, moves, array) {
...
// function moveDown(x, y, moves, array) {
//   for (let i = 0; i <= moves; i++) {
//     y--;
//     array[x][y].push(true);
//   }
...
// function moveRight(x, y, moves, array) {
//   for (let i = 0; i <= moves; i++) {
//     x++;
//     array[x][y].push(true);
//   }
...
// function moveLeft(x, y, moves, array) {
//   for (let i = 0; i <= moves; i++) {
//     x--;
//     array[x][y].push(true);
//   }
